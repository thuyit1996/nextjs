...
mock.onPost('/api/scrumboard/add/card').reply((request: AxiosRequestConfig) => {
  ...
  let selectedBoard: BoardObj = boardData.find(data => data.id === board.id)!;
  let selectedList: CardListObj = selectedBoard.list.find(
...
mock
  ...
  .reply((request: AxiosRequestConfig) => {
    ...
    let selectedBoard: BoardObj = boardData.find(data => data.id === boardId)!;
    let selectedList: CardListObj = selectedBoard.list.find(
      data => data.id === listId,
    )!;
